...
export class Scene extends PHASER.Scene {
    ...
    public preload() {
        ...
        this.load.spritesheet('character', 'assets/gfx/character/character.png', { frameWidth: 48, frameHeight: 64 });
    }
    ...
    public create() {
        ...
        this.anims.create({
            key: 'walk_up',
            frames: this.anims.generateFrameNumbers('character', { frames: [ 18, 19, 20 ] }),
            frameRate: 8,
            repeat: -1
        });
        ...
        this.anims.create({
            key: 'walk_right',
            frames: this.anims.generateFrameNumbers('character', { frames: [ 12, 13, 14 ] }),
            frameRate: 8,
            repeat: -1
        });
         ...
         */
        this.player = this.physics.add.sprite(96, 32, 'character', 0).setScale(1.5);
        this.player.setCollideWorldBounds(true);
        this.initializeBall();

        // this.player.setCollideWorldBounds(); TODO: Fix wrong world bounds
        this.debugText = this.add.text(25, 25, `Player position: ${this.player.x}, ${this.player.y}`);
    ...
    public update(_time: number, _delta: number) {
        ...
        if(this.keyboard.left.isUp && this.keyboard.right.isUp && this.keyboard.down.isUp && this.keyboard.up.isUp){
            this.player.stop();
        }
        ...
        this.debugText.setText(`Player position: ${Math.ceil(this.player.x / 64)}, ${Math.ceil(this.player.y / 64)}`);
    }
    createDatGUI(){
        ...
        this.createVectorGui(folderBall, 'Acceleration', this.ball.body.acceleration, -600, 600, 10);
        this.createVectorGui(folderBall, 'Bounce', this.ball.body.bounce, 0, 1, 0.1);
        this.createVectorGui(folderBall, 'deltaMax', this.ball.body.deltaMax, 0, 60, 1);
        this.createVectorGui(folderBall, 'drag', this.ball.body.drag, 0, 60, 0.1);
        this.createVectorGui(folderBall, 'friction', this.ball.body.friction, 0, 1, 0.05);
        this.createVectorGui(folderBall, 'gravity', this.ball.body.gravity, -600, 600, 10);
        this.createVectorGui(folderBall, 'maxVelocity', this.ball.body.maxVelocity, 0, 10000, 100);
    ...
    initializeBall(){
        this.ball = this.physics.add.sprite(16 * 64, 7 * 64, 'ball', 0).setScale(0.2);
